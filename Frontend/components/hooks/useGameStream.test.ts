...
describe('useGameStream', () => {
  ...
  it('should update player names on playerJoined event', () => {
    ...
    const playerJoinedCallback = vi.mocked(mockEventSource.addEventListener).mock.calls.find(call => call[0] === 'playerJoined')?.[1] as Function
  ...
  it('should update gameStarted on gameStarted event', () => {
    ...
    const gameStartedCallback = vi.mocked(mockEventSource.addEventListener).mock.calls.find(call => call[0] === 'gameStarted')?.[1] as Function
  ...
  it('should handle attackResult event with delay', () => {
    ...
    const attackResultCallback = vi.mocked(mockEventSource.addEventListener).mock.calls.find(call => call[0] === 'attackResult')?.[1] as Function
